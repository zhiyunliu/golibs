...
func TestPathMatch_spec_Match(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			m := NewMatch([]Pattern{p, StrPattern("/")})
			...
			match2, pattern2 := m.Match(tt.args.path, tt.args.spl)
			if tt.want.match != match2 {
				t.Errorf("name:%s,expectMatch:%v,actual:%v,pattern:%s,path:%s", tt.name, tt.want.match, match2, pattern2, tt.args.path)
			}
		})
	}
}
